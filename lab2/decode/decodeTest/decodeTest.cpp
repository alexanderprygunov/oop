...
BOOST_AUTO_TEST_SUITE(HtmlDecode_function)
	...
	{
		string noCoding = "police";
		noCoding = HtmlDecode(noCoding);
		BOOST_CHECK(StringsAreEqual(noCoding, "police"));
	}

	BOOST_AUTO_TEST_CASE(string_repeated_symbol)
	{
		string repeatedHtmlSymbol = "hello &quot;&quot; world";
		repeatedHtmlSymbol = HtmlDecode(repeatedHtmlSymbol);
		BOOST_CHECK(StringsAreEqual(repeatedHtmlSymbol, "hello \"\" world"));
	}
	
	BOOST_AUTO_TEST_CASE(string_with_all_html_symbols)
	{
		string html = "Mother &lt;says&gt; &quot;Hello&quot;! M&amp;M&apos;s";
		html = HtmlDecode(html);
		BOOST_CHECK(StringsAreEqual(html, "Mother <says> \"Hello\"! M&M's"));
	}
